...
function links(items, empty) {
  ...
  return items
    .map((x) => {
      const extra = x.vote_data === "none" ? " · partiröst okänd" : "";
      return `<a href="${x.url}" rel="noopener">${x.label}</a>${x.date ? ` <span class="empty">${x.date}</span>` : ""}${extra}`;
    })
    .join("<br />");
}
...
function render(data) {
  ...
  const frz = data.kpis && data.kpis.freeze_match;
  const cov = (data.coverage && data.coverage.motions_title_gated_by_rm) || {};
  document.getElementById("meta").innerHTML =
    `Topic: <strong>AI</strong> · rm ${ (data.windows || []).join(" · ") } · locator ${loc == null ? "—" : Math.round(loc * 100) + "%"} · freeze-match ${frz == null ? "—" : Math.round(frz * 100) + "%"} · titel-gate 23/24:${cov["2023/24"] ?? 0} 24/25:${cov["2024/25"] ?? 0} 25/26:${cov["2025/26"] ?? 0}`;

  document.getElementById("tvn").innerHTML = (data.then_vs_now || [])
    ...
    .join("") || `<div class="tvn-card">then_vs_now: underlag saknas</div>`;
  ...
  document.getElementById("grid").innerHTML = data.parties
    .map(
      (p) => `<article class="card">
        ...
        <p class="row"><span class="k">Röstade</span>${links(p.votes, "underlag saknas")}</p>
